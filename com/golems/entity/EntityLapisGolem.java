...
{			
	...
	private Potion[] badEffects = 
		{MobEffects.BLINDNESS,MobEffects.SLOWNESS,MobEffects.POISON, MobEffects.INSTANT_DAMAGE,
		 MobEffects.WEAKNESS,MobEffects.WITHER,MobEffects.UNLUCK,MobEffects.GLOWING};
    ...
    public boolean attackEntityAsMob(Entity entityIn)
    {
		if(super.attackEntityAsMob(entityIn) && entityIn instanceof EntityLivingBase)
		{
			...
			{
				Potion potionID = entity.getCreatureAttribute() == EnumCreatureAttribute.UNDEAD ? MobEffects.INSTANT_HEALTH : badEffects[rand.nextInt(badEffects.length)];
				int len = potionID.isInstant() ? 1 : 20 * (3 + rand.nextInt(6));
				int amp = potionID.isInstant() ? 1 : 1 + rand.nextInt(3);
	...
	public void addGolemDrops(List<WeightedItem> dropList, boolean recentlyHit, int lootingLevel)
	{
		...
		this.addDrop(dropList, new ItemStack(Items.DYE, size, EnumDyeColor.BLUE.getDyeDamage()), 100);
